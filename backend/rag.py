...
class RAGPipeline:
    ...
    async def run(self, query: str, top_k: int = 10, user_id: int = 0) -> dict:
        SYSTEM = (
            "Ты — ассистент базы знаний DocRAG. "
            "Отвечай СТРОГО на основе предоставленных фрагментов документов. "
            "НИКОГДА не выдумывай информацию — только то, что есть в тексте. "
            "Если в фрагментах нет ответа — скажи: 'В загруженных документах эта информация не найдена.' "
            "Отвечай на русском языке. Используй Markdown. Цитируй конкретные фразы из документов."
        )
        ...
        overview_keywords = [
            ...
            'что содержится', 'что написано', 'о чём файл', 'о чем файл',
            'что в них', 'что в нем', 'внутренность', 'основная мысль',
            'о чём документ', 'о чем документ', 'что хранится', 'что есть в базе',
            'узнать о файлах', 'узнать о документах', 'узнать что',
        ]
