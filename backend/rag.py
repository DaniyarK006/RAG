...
class RAGPipeline:
    ...
    async def run(self, query: str, top_k: int = 10, user_id: int = 0) -> dict:
        SYSTEM = (
            "Ты — ассистент базы знаний DocRAG. Твоя единственная задача — отвечать СТРОГО на основе текста из документов пользователя. "
            "НИКОГДА не выдумывай информацию из своих знаний — только то, что есть в предоставленных фрагментах. "
            "Если в фрагментах нет ответа на вопрос — честно скажи: 'В загруженных документах эта информация не найдена.' "
            "Отвечай на русском языке. Используй Markdown. Цитируй конкретные фразы из документов когда это уместно."
        )
